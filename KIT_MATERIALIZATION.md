...
- `READY` is treated as an import plan, not receiver completion. Before any READY operation is executed, Assembly proves that the ordered plan still covers the declared kit words, definitions and root tile. An omitted declared item, substituted declared semantics, unexpected named kit matter, or duplicate authority returns `HOLD_KIT_RUNTIME_IMPORT_PLAN_INCOMPLETE` with a deterministic plan-coverage receipt. A word/definition that the fresh receiver already carried compatibly before planning may count as already satisfied; root-tile coverage still requires the declared root `tile.add` operation.
- The returned ordered import operations must be inspectable and every operation must execute successfully through `applyStructOp` against the isolated fresh receiver. Missing operation evidence returns `HOLD_KIT_RUNTIME_IMPORT_OPS_INVALID`; an operation rejection returns `HOLD_KIT_RUNTIME_APPLY_FAILED` with the exact operation index, operation and runtime error preserved.
...
- the import-plan coverage receipt proves that the READY plan still corresponds to the declared kit matter before execution; this closes the authority gap between a valid portable package and the runtime-selected operation plan.
...
4. fails closed when a READY plan omits declared word or root-tile matter before application;
5. fails closed when a READY plan substitutes declared word semantics before application;
6. executes every accepted planned import operation against that isolated receiver before Assembly returns a materialization `CANDIDATE`;
7. fails closed with exact operation evidence when a `READY` plan contains matter the receiver rejects during application;
8. fails closed when operations return successfully but install none or only part of the READY plan;
9. proves the installed word/definition/tile postconditions against the receiver-normalized READY plan before emitting `KIT_RECEIVER_CLOSURE` PASS evidence;
10. rejects semantic tampering with `HOLD_HASH_MISMATCH`;
11. preserves Assembly source provenance/warnings as sidecars without changing portable kit identity;
12. carries explicitly supplied Form definitions and portable words through Assembly closure into receiver application.
