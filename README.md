...
- IMPLEMENTED ON MAIN: deterministic tile/facet/capability folding, fail-closed upstream-envelope and authored-wrapper handling, exact request/intent grammar, semantic container validation across wrapped and direct candidate paths, authored-presence source tracing, request/input closure collection, source provenance, upstream-warning/HOLD preservation, canonical closure hashing, exact addressed Interface view/presentation folding, schema gating, source-integrity preflight, lossless conflict evidence, named own-key world closure, generic own-key candidate merge identity, READY import-plan coverage, and runtime-backed MorphTile kit materialization with bounded local Interface proof discharge plus receiver-application/postcondition closure.
- CURRENT FLEET RECEIPT: `fixtures/current-fleet.json` is the executable identity receipt for the currently integrated Form, Surface, Capability, Interface, and MorphTile core revisions. Exact-head CI and independent Verification evidence stays on PRs/handoffs instead of being copied into this merge-stable section.
- PINNED TEST HARNESS: historical exact receipts plus separately pinned receiver lanes remain historical evidence. Current-fleet compatibility is re-earned through the executable receipt rather than relabelling old receipts.
...
- NOT CLAIMED: compatibility outside exact evidenced receipts, unintegrated sibling semantics, arbitrary new MorphTile facet vocabulary, arbitrary operation composition, automatic definition discovery/fetch, automatic `ui_panel` invention, visual quality, automatic CANON, or merge authority.
- HELD: unintegrated sibling/core candidates remain outside Assembly authority; unresolved arbitrary dependency transport through MorphTile kits remains explicit; conflict auto-resolution remains intentionally absent; presentation z-order/layering remains HOLD until MorphTile exposes an evidenced canonical representation/runtime primitive.
