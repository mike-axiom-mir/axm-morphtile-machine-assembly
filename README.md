...
- CURRENT CANDIDATE: READY import-plan coverage. After `importKit: READY` and before application, Assembly verifies that the runtime-selected plan still covers the declared kit words, definitions and root tile without omitted or substituted source matter; failures return `HOLD_KIT_RUNTIME_IMPORT_PLAN_INCOMPLETE` rather than allowing plan-relative receiver proof to hide source omissions.
