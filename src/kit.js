...
function hold(code, detail, fields = {}) {
  return {
    status: "HOLD",
    kit: null,
    source_closure_hash: fields.source_closure_hash || null,
...
function materializeKit(assemblyResult, runtime, options = {}) {
  ...
  if (dependencies.length) {
    return hold("HOLD_KIT_DEPENDENCY_UNREPRESENTABLE", "MorphTile kit v0.x does not carry Assembly's arbitrary dependency records; refusing to silently drop dependency closure.", {
      source_closure_hash: clone(assemblyResult.closure_hash),
  ...
  return {
    ...
    evidence: [
      ...
      { kind: "KIT_HASH", status: "PASS", check: "kit expect.sha256 uses the supplied MorphTile runtime hashOf over tile + defs + words" },
