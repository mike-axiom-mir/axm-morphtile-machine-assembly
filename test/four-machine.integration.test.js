...
test("four pinned sibling outputs expose the ui_panel compatibility boundary and assemble once it is explicitly supplied", { skip: !ready }, () => {
  ...
  const form = Form.run(req("form-four", "Create the proof form", {
    shape: "box",
    name: "Four-machine proof",
    size: [1, 1, 1]
  }));
  const surface = Surface.run(req("surface-four", "Create the proof surface", {
    base_color: [0.3, 0.4, 0.5]
  }));
  const capability = Capability.run(req("capability-four", "Create the proof counter behavior", {
    kind: "counter"
  }));
  const interfaceOut = Interface.run(req("interface-four", "Create the proof counter interface", {
    tile_path: "mt_four_machine",
    title: "Counter",
    readout: "count",
    action: "increment"
  }));
  ...
  assert.equal(interfaceOut.candidate.schema, "morphtile.view-operation/v0.4");
  assert.equal(interfaceOut.warnings.some((warning) => warning.code === "TARGET_MUST_EXIST_AND_DECLARE_UI_PANEL"), true);

  const raw = assemble({
    envelope_version: "0.1",
    request_id: "assemble-four-raw",
    goal: "Assemble the four current stable specialist outputs",
    intent: { id: "mt_four_machine", name: "Four-machine proof" },
  ...
  const combined = assemble({
    envelope_version: "0.1",
    request_id: "assemble-four-compatible",
    goal: "Assemble the four current stable specialist outputs with explicit ui_panel eligibility",
    intent: { id: "mt_four_machine", name: "Four-machine proof" },
  ...
  assert.equal(combined.candidate.id, "mt_four_machine");
